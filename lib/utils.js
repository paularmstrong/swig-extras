...
/**
 * Reapeat a string pattern a certain number of times
 * @param {string} [pattern] to be repeated
 * @param {number} [count] number of times to repeat pattern
 */

exports.repeat = function (pattern, count) {
  if (count < 1) {
    return '';
  }
 ...
 * @private
 */
exports.iterateFilter = function (input) {
  var self = this,
    out = {};

  if (exports.isArray(input)) {
    return exports.map(input, function (value) {
      return self.apply(null, arguments);
    });
  }

  if (typeof input === 'object') {
    exports.each(input, function (value, key) {
